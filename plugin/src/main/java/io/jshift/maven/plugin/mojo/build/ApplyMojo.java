...
import io.jshift.kit.config.service.ApplyService;
...
public class ApplyMojo extends AbstractJshiftMojo {
    ...
    public static final String DEFAULT_KUBERNETES_MANIFEST = "${basedir}/target/classes/META-INF/jshift/kubernetes.yml";
    public static final String DEFAULT_OPENSHIFT_MANIFEST = "${basedir}/target/classes/META-INF/jshift/openshift.yml";

    /**
     * The domain added to the service ID when creating OpenShift routes
     */
    @Parameter(property = "jshift.domain")
    protected String routeDomain;

    /**
     * Should we fail the build if an apply fails?
     */
    @Parameter(property = "jshift.deploy.failOnError", defaultValue = "true")
    protected boolean failOnError;

    /**
     * Should we update resources by deleting them first and then creating them again?
     */
    @Parameter(property = "jshift.recreate", defaultValue = "false")
     ...
     */
    @Parameter(property = "jshift.deploy.create", defaultValue = "true")
    private boolean createNewResources;

    /**
     * Should we use rolling upgrades to apply changes?
     */
    @Parameter(property = "jshift.rolling", defaultValue = "false")
    private boolean rollingUpgrades;

    /**
     * Should we fail if there is no kubernetes json
     */
    @Parameter(property = "jshift.deploy.failOnNoKubernetesJson", defaultValue = "false")
     ...
     */
    @Parameter(property = "jshift.deploy.servicesOnly", defaultValue = "false")
     ...
     * the service.
     */
    @Parameter(property = "jshift.deploy.ignoreServices", defaultValue = "false")
     ...
     */
    @Parameter(property = "jshift.deploy.processTemplatesLocally", defaultValue = "false")
    private boolean processTemplatesLocally;

    /**
     * Should we delete all the pods if we update a Replication Controller
     */
    @Parameter(property = "jshift.deploy.deletePods", defaultValue = "true")
     ...
     */
    @Parameter(property = "jshift.deploy.ignoreRunningOAuthClients", defaultValue = "true")
     ...
     * <a href="https://github.com/jshiftio/exposecontroller/">exposecontroller</a> to decide
     * if Ingress or Router is being used or whether we should use LoadBalancer or NodePorts for single node clusters
     */
    @Parameter(property = "jshift.deploy.createExternalUrls", defaultValue = "false")
    private boolean createExternalUrls;

    /**
     * The folder we should store any temporary json files or results
     */
    @Parameter(property = "jshift.deploy.jsonLogDir", defaultValue = "${basedir}/target/jshift/applyJson")
    private File jsonLogDir;

    /**
     * How many seconds to wait for a URL to be generated for a service
     */
    @Parameter(property = "jshift.serviceUrl.waitSeconds", defaultValue = "5")
    ...
    /**
     * Environment name where resources are placed. For example, if you set this property to dev and resourceDir is the default one, jshift will look at src/main/jshift/dev
     * Same applies for resourceDirOpenShiftOverride property.
     */
    @Parameter(property = "jshift.environment")
    private String environment;

    @Parameter(property = "jshift.skip.apply", defaultValue = "false")
    ...
    protected String getExternalServiceURL(Service service) {
        return KubernetesHelper.getOrCreateAnnotations(service).get(JshiftAnnotations.SERVICE_EXPOSE_URL.value());
    }
